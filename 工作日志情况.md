...
- **[16:55] Project Initialization**
    ...
    - **[2025-12-26 01:14] 🔧 32B 模型 OOM 问题解决 & 模型缩放**
        ...
        - **后台下载**: `model_download_dashboard.py` 面板已上线 (`http://127.0.0.1:9877`)，支持可视化监控。
        - **下一步**: 模型下载完成后，使用 14B 模型重启训练

    - **[2025-12-26 17:55] 模型库清理与 TTS 方案拓展**
        - **[下载]** 修复下载脚本的代理泄露问题，强制 Clash 内核直连 `hf-mirror` 相关域名。
        - **[清理]** 移除了 `Qwen(2.5/3)-32B` 系列模型文件，释放空间并聚焦于 14B 模型。
        - **[Dashboard]** 优化了监控面板，支持正确识别 Fake-IP 直连状态。
        - **[TTS]** 启动新一代 TTS 评估，正在部署 `CosyVoice`, `ChatTTS`, `F5-TTS` 以对比情感表现。
    - **[2025-12-26 18:02] 音频生成测试 (GPT-SoVITS)**
        - **输入**: "夏生さん、見てください！この花、綺麗でしょう？..."
        - **参考音频**: `atr_b112_073.wav` (自信/开心语气)
        - **输出文件**: `ATRI_Test_HighQuality_20251226_180231.wav`
        - **修正**: 严格遵守必读文件规定，输出文件名必须包含时间戳，防止覆盖。
...
- **[02:18] 模型下载完成确认 ✅**
    - **已下载模型库** (总计约 265GB):
        | 模型 | 大小 | 状态 |
        |------|------|------|
        | Qwen3-32B-final | 62G | ✅ 完整 |
        | Ministral-3-14B-Instruct | 30G | ✅ 完整 |
        | Qwen3-14B-Base | 28G | ✅ 完整 |
        | Qwen2.5-14B-Roleplay-ZH | 28G | ✅ 完整 |
        | DeepSeek-R1-Distill-Qwen-14B | 28G | ✅ 完整 |
        | Aris-Qwen1.5-14B-DPO | 27G | ✅ 完整 |
        | Qwen2.5-14B-MegaFusion-RP | 26G | ✅ 完整 |
        | DeepSeek-R1-Distill-Qwen-32B | 19G | ⚠️ 不完整 (无用) |
        | Yi-1.5-9B-Chat | 17G | ✅ 完整 |
    - **后台 Dashboard**: 进程 PID 702925 已完成任务，可关闭。

- **[02:20] 🧠 LLM-Enhanced GPT-SoVITS 优化计划启动**
    - **目标**: 使用 LLM 提升 GPT-SoVITS 的**情感表达能力**和**韵律自然度**。
    - **选定模型**: **Qwen2.5-14B-Roleplay-ZH** (角色扮演专精，28GB)
        - 选择理由：专为中文角色扮演优化，情感理解能力强，适合生成带有情感标注的文本。
    - **优化策略**:
        1. **情感预测 (Emotion Prediction)**: 使用 LLM 分析输入文本，预测情感标签 (喜悦/悲伤/愤怒/惊讶/平静)。
        2. **韵律标注 (Prosody Tagging)**: LLM 为文本添加停顿、重音、语速等韵律标记。
        3. **参考音频智能选择**: 根据情感预测，自动从 ATRI 音频库中匹配最佳参考音频。
    - **技术方案**: 创建 `llm_enhanced_tts.py` 流水线脚本。

- **[16:00] 🔊 超高质量合成流程确立**
    - ✅ 创建 `hq_tts_synthesis.py`，优化参数：
        - `top_k=5`, `top_p=0.8`, `temperature=0.5`
        - 参考音频时长严格控制在 3-10s
    - ✅ 构建情感参考库 `reference_library.json` (2154 样本, 9 种情感)
    - ✅ 首次 HQ 合成成功：`hq_happy_20251227_160550.wav`

- **[16:10] 🚀 v4 预训练模型下载**
    - ✅ `s2Gv4.pth` (769MB) - SoVITS v4 底模
    - ✅ `vocoder.pth` (57MB) - HiFiGAN 声码器

- **[16:20] ⚡ 双卡 v4 训练启动**
    - **GPU 0**: SoVITS v4 (Batch 24, 10 epochs) - 20GB VRAM, 100%
    - **GPU 1**: GPT v4 + DPO (Batch 8, 20 epochs) - 12GB VRAM, 78%
    - **训练数据**: 2154 条 → 2083 条有效样本
    - **训练脚本**: `train_sovits_v4.sh`, `train_gpt_v4.sh`

- **[16:25] 🧪 自动评测脚本**
    - ✅ 创建 `evaluate_checkpoints.py`
    - 功能: 5 种情感 × 2 句测试，自动对比不同 checkpoint

- **[16:35] 💡 动态 RAG 提示词设计 (封神级功能)**
    - **概念**: LLM 不仅输出 [Emotion]，还输出 [Speed], [Pitch] 等 TTS 控制参数
    - **示例输出**:
      ```
      [Emotion=Sad, Speed=0.85, Pitch=-2]
      我......我也想和夏生先生一直在一起......
      ```
    - **应用场景**:
      | 场景 | 参数 |
      |------|------|
      | 能量不足/低语 | `Speed=0.85, Pitch=-1` |
      | 兴奋/骄傲 | `Speed=1.1, Pitch=+1` |
      | 害羞/小声 | `Speed=0.9, Pitch=0, Volume=0.7` |
    - **实现路径**: 修改 `hq_tts_synthesis.py` 解析 LLM 输出标签
